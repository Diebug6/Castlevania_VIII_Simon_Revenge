function heart(x, y){
    ...
    this.height = 30;
    this.width = 30;
	this.sfx = new sound("assets/sounds/Effects/Others/powerup.ogg");
    ...
    this.update = function () {
        ...
        if (colission(gameObjects[player].x, gameObjects[player].y, gameObjects[player].width, gameObjects[player].height, this.x, this.y, this.width, this.height)) {
            this.sfx.play();
			this.x = -1000;
            gameObjects[player].hearts++;
            gameObjects[player].points += 200;
        }
    }},

    this.render = function () {
        this.ctx = cont;
        this.ctx.drawImage(images.heart[0], this.x, this.y);
    }
}